...
-import(utils, [format_timestamp/1, format_mac_addr/1, format_ethernet_proto_type/1, format_ip_proto_type/1, format_ip_addr/1]).

-author({guoguo, "goosman.lei@gmail.com"}).
-purpose("unpack package from tcpdump capture").
...
unpack(Content) ->
    ...
    >> = PCAP_FILE_Body,
    io:format("PCAP_Packet_Timestamp_Second: ~p[~s]~n", [PCAP_Packet_Timestamp_Second, format_timestamp(PCAP_Packet_Timestamp_Second)]),
    ...
    >> = PCAP_Packet_Body,
    io:format("Ethernet_DstAddr: ~s~n", [format_mac_addr(Ethernet_DstAddr)]),
    io:format("Ethernet_SrcAddr: ~s~n", [format_mac_addr(Ethernet_SrcAddr)]),
    io:format("Ethernet_Type: ~4.16.0B ~p~n", [Ethernet_Type, format_ethernet_proto_type(Ethernet_Type)]),
    ...
    io:format("IP_Protocol: ~s~n", [format_ip_proto_type(IP_Protocol)]),
    io:format("IP_HeaderCheckSum: 0X~.16B [~p]~n", [IP_HeaderCheckSum, IP_HeaderCheckSum]),

    io:format("IP_SrcAddr: ~s~n", [format_ip_addr(IP_SrcAddr)]),
    io:format("IP_DstAddr: ~s~n", [format_ip_addr(IP_DstAddr)]),
